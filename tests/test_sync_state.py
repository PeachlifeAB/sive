...
def test_run_sync_vault_drains_pending_queue_on_success(tmp_path):
    now = datetime(2026, 4, 1, 22, 0, tzinfo=UTC)
    with (
        patch.object(sync_state, "STATE_DIR", tmp_path),
        ...
        patch.object(sync_state, "drain_pending") as mock_drain,
        patch.object(sync_state, "_write_snapshot_from_session"),
    ):
        sync_state.run_sync_vault("personal")

    mock_drain.assert_called_once_with("personal", "session", "/tmp/sive-personal")
...
def test_run_sync_vault_updates_success_state(tmp_path):
    ...
    with (
        ...
        patch.object(sync_state, "drain_pending"),
        patch.object(sync_state, "_write_snapshot_from_session"),
    ):
