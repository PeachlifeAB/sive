...
def verify_installed_tool(version: str) -> None:
    """Install sive into a throwaway venv and confirm --version.

    A temp venv is used on purpose so the check never installs into ~/.local/bin,
    which would shadow a Homebrew install. The venv is removed automatically.
    """
    with tempfile.TemporaryDirectory(prefix="sive-verify-") as tmp:
        venv = Path(tmp) / "venv"
        ...
        executable = venv / "bin" / PROJECT_NAME
        version_output = subprocess.run(
