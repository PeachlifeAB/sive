...
def run() -> int:
    ...
    if not hook_configured:
        ...
        _echo("  - global (default, no .sive in this directory)")
    ...
    if not hook_configured:
        _echo("Warning: sive env hook not configured in mise.", file=sys.stderr)
    ...
    if not hook_configured or not cache_enabled or not server_matches:
        return 1
...
def _read_mise_state() -> tuple[bool, bool, str]:
    ...
    env = data.get("env", {})
    hook_configured = isinstance(env, dict) and "_.source" in env and "sive" in str(env["_.source"])
    return (
        hook_configured,
